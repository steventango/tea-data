...
class DICT {
    ...
    async load() {
        try {
            ...
            const [cedict_ts, cccedict_canto_readings, ccccanto_webdist, wordlist] = await Promise.all([
            ...
            data = data.concat(ccccanto_webdist
                .split('\n')
                .filter((line) => !line.startsWith('#'))
                .map((line) => this.parse_line(line))
                .filter((entry) => entry !== null && entry !== undefined));
            const augmented_data = new Map(cccedict_canto_readings
                .split('\n')
                .filter((line) => !line.startsWith('#'))
                .map((line) => this.parse_line(line))
                .filter((entry) => entry !== null && entry !== undefined)
