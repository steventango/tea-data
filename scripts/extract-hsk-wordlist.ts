...
interface ParsedHSKWord {
  word: string;
  h: number;
}

interface ExtractedHskWordlist {
  generatedAt: string;
  source: {
    pdfPath?: string;
    textPath?: string;
...
interface ParseDiagnostics {
  ...
  parsedRows: number;
  skippedRows: number;
  invalidSerialRows: number;
  invalidLevelRows: number;
  unsupportedLevelRows: number;
  emptyWordRows: number;
  duplicateRows: number;
}

interface ParsedHskText {
  words: Array<ParsedHSKWord>;
  count: number;
...
function parseHskLevelToken(levelToken: string): number | undefined {
...
function parseHskText(rawText: string): ParsedHskText {
  ...
  const words: Array<ParsedHSKWord> = Array.from(byWord.entries())
...
function printQualityReport(parsed: ParsedHskText, report: QualityReport) {
  ...
  console.log(`- parse failures: ${
    report.parseDiagnostics.invalidSerialRows + report.parseDiagnostics.invalidLevelRows + report.parseDiagnostics.unsupportedLevelRows
  }`);
...
function validateParsedQuality(parsed: ParsedHskText, strict = false) {
...
async function extractPdfToText(pdfPath: string, layout = false): Promise<string> {
...
async function extractHskWords(args: CliArgs) {
  let rawText: string;
  const source = {
    pdfPath: args.pdfPath,
    textPath: args.textPath,
