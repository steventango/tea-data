...
function readHeaderMetadata(lines) {
...
function parseUpstreamMetadata(text, headers) {
...
async function fetchCedictArchive(url, redirects = 0) {
    if (redirects > 5) {
...
async function decompressGzip(payload) {
    const chunks = [];
    return new Promise((resolve, reject) => {
...
async function localHash(filePath) {
    try {
...
async function summarizePartitions(partitionsPath) {
    const partitions = JSON.parse(await fs.readFile(partitionsPath, 'utf8'));
...
//# sourceMappingURL=update-cedict-partitions.js.map